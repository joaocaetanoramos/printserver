...
MSG_INST_STEP_IP="Pinning the current IP so it stays after a reboot (set PS_NO_STATIC_IP=1 to skip)..."
...
MSG_WIFI_FAIL="Could not connect to '%s'. Check the network name (case-sensitive), the password, and the security type (WPA2/WPA3)."
...
MSG_IP_DHCP_OK="Back to DHCP — the IP now comes from your router."
MSG_IP_LINKLOCAL="! Refusing to pin a link-local/loopback address. Check the network first."
